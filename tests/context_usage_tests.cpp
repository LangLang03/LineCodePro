...
void GroupedTokensMatchNumberFormat() {
  ...
  EXPECT_EXPRESSION(FormatGroupedTokens(0) == "0");
  EXPECT_EXPRESSION(FormatGroupedTokens(7) == "7");
  EXPECT_EXPRESSION(FormatGroupedTokens(999) == "999");
  EXPECT_EXPRESSION(FormatGroupedTokens(1'000) == "1,000");
  EXPECT_EXPRESSION(FormatGroupedTokens(4'096) == "4,096");
  EXPECT_EXPRESSION(FormatGroupedTokens(25'000) == "25,000");
  EXPECT_EXPRESSION(FormatGroupedTokens(250'000) == "250,000");
  EXPECT_EXPRESSION(FormatGroupedTokens(1'000'000) == "1,000,000");
  EXPECT_EXPRESSION(FormatGroupedTokens(-12'345) == "-12,345");
}

void FormatLabelMatchesLegacy() {
  EXPECT_EXPRESSION(linecode::domain::FormatContextLabel(250'000) == "250K");
  EXPECT_EXPRESSION(linecode::domain::FormatContextLabel(1'000'000) == "1M");
  EXPECT_EXPRESSION(linecode::domain::FormatContextLabel(4'096) == "4096");
  EXPECT_EXPRESSION(linecode::domain::FormatContextLabel(0).empty());
}
...
void ModelIdSuffixIsParsedLikeModelContextParser() {
  ...
  EXPECT_EXPRESSION(plain.api_model_id == "gpt-4o");
  EXPECT_EXPRESSION(plain.context_tokens == linecode::domain::default_context_tokens);
  EXPECT_EXPRESSION(plain.context_label == "250K");

  // Lower-case and upper-case units both scale.
  auto kilo = linecode::domain::ResolveModelContext(std::string_view{"m[128k]"});
  EXPECT_EXPRESSION(kilo.api_model_id == "m");
  EXPECT_EXPRESSION(kilo.context_tokens == 128'000);
  EXPECT_EXPRESSION(kilo.context_label == "128K");

  auto mega = linecode::domain::ResolveModelContext(std::string_view{"m[1M]"});
  EXPECT_EXPRESSION(mega.context_tokens == 1'000'000);

  // A fractional value rounds like Math.round.
  auto fractional =
      linecode::domain::ResolveModelContext(std::string_view{"m[1.5k]"});
  EXPECT_EXPRESSION(fractional.context_tokens == 1'500);

  // A suffix without a unit is a literal token count.
  auto literal = linecode::domain::ResolveModelContext(std::string_view{"m[4096]"});
  EXPECT_EXPRESSION(literal.context_tokens == 4'096);

  // A malformed suffix is not a suffix: the model keeps the whole id.
  auto malformed =
      linecode::domain::ResolveModelContext(std::string_view{"m[abc]"});
  EXPECT_EXPRESSION(malformed.api_model_id == "m[abc]");
  EXPECT_EXPRESSION(malformed.context_tokens == linecode::domain::default_context_tokens);
  ...
  EXPECT_EXPRESSION(bare.api_model_id == "[8k]");
  EXPECT_EXPRESSION(bare.context_tokens == 8'000);
}
...
void ExplicitContextSizeWinsOverSuffix() {
  ...
  EXPECT_EXPRESSION(resolved.api_model_id == "m[8k]");
  EXPECT_EXPRESSION(resolved.context_tokens == 32'000);
  EXPECT_EXPRESSION(resolved.context_label == "32K");
  ...
  EXPECT_EXPRESSION(fallen.api_model_id == "m");
  EXPECT_EXPRESSION(fallen.context_tokens == 8'000);
}
...
void EstimationMirrorsContextManager() {
  ...
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(blank, true) == 0);

  // Overhead plus ceil(characters / 4).
  auto short_message = UserMessage("abcd");
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(short_message, true) == 8 + 1);

  auto long_message = UserMessage(std::string(400, 'x'));
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(long_message, true) == 8 + 100);

  // Excluded messages cost nothing even with content.
  auto excluded = UserMessage("ignored");
  excluded.exclude_from_context = true;
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(excluded, true) == 0);

  // Reasoning is counted only when requested.
  ChatMessage reasoned = UserMessage("abcd");
  reasoned.reasoning_content = std::string(40, 'r');
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(reasoned, true) == 8 + 1 + 10);
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(reasoned, false) == 8 + 1);

  // Tool messages are context messages even with empty content.
  ChatMessage tool;
  tool.role = MessageRole::tool;
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(tool, true) == 8);
  ...
  const int attachment_cost = attached.attachments.size() == 0
                                  ...
                                  : linecode::domain::EstimateMessageTokens(
                                        attached, true);
  EXPECT_EXPRESSION(attachment_cost > 8 + 1);
}
...
void ToolCallArgumentsAndResultsAreCounted() {
  ...
  EXPECT_EXPRESSION(with_result > linecode::domain::message_overhead_tokens);

  // Dropping the result lowers the estimate, which proves the result text is
  // part of the cost.
  message.timeline.clear();
  message.content = "x";
  EXPECT_EXPRESSION(linecode::domain::EstimateMessageTokens(message, true) < with_result);
}
...
void SnapshotClampsPercent() {
  ...
  EXPECT_EXPRESSION(small.max_tokens == 10'000);
  EXPECT_EXPRESSION(small.used_tokens > 0);
  EXPECT_EXPRESSION(small.percent > 0 && small.percent < 100);
  ...
  EXPECT_EXPRESSION(saturated.percent == 100);
  ...
  EXPECT_EXPRESSION(guarded.max_tokens == 1);

  // No messages at all reports zero percent.
  const auto empty =
      linecode::domain::SnapshotContext({}, 10'000, true);
  EXPECT_EXPRESSION(empty.used_tokens == 0);
  EXPECT_EXPRESSION(empty.percent == 0);
}

} // namespace

TEST(context_usage_tests, LegacySuite) {
