...
huxerui::View Probe() {
  ...
  huxerui::Lifecycle([scenario, tasks] {
    const auto handle = tasks.Launch([scenario]() -> huxerui::Task<void> {
      ...
      assert(Near(migrated, 1.25F));
      assert(scenario->store->integers.at("markdown_text_scale") == 1'250);
      assert(scenario->store->integer_writes == 1);

      const float preserved = co_await scenario->reading->LoadScale(0.75F);
      assert(Near(preserved, 1.25F));
      assert(scenario->store->integer_writes == 1);

      co_await scenario->reading->SaveScale(9.0F);
      assert(scenario->store->integers.at("markdown_text_scale") == 1'600);
      const float clamped = co_await scenario->reading->LoadScale(0.75F);
      assert(Near(clamped, 1.6F));
      ...
      assert(Near(defaulted, 1.0F));
      assert(scenario->store->integer_writes == writes_before_default);
      scenario->done = true;
    });
...
int main() {
  assert(SkillHubReadingSettings::NormalizeScale(0.0F) == 0.5F);
  assert(SkillHubReadingSettings::NormalizeScale(0.5F) == 0.5F);
  assert(SkillHubReadingSettings::NormalizeScale(1.25F) == 1.25F);
  assert(SkillHubReadingSettings::NormalizeScale(1.6F) == 1.6F);
  assert(SkillHubReadingSettings::NormalizeScale(2.0F) == 1.6F);
  assert(SkillHubReadingSettings::NormalizeScale(
