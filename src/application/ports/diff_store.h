...
public:
  ...
  [[nodiscard]] virtual huxerui::Task<domain::DiffRecord>
  Record(std::string file_path, std::string old_content,
         std::string new_content, bool old_exists) = 0;

  [[nodiscard]] virtual huxerui::Task<std::optional<domain::DiffRecord>>
  ...
  [[nodiscard]] virtual huxerui::Task<std::vector<domain::DiffRecord>>
  ...
  [[nodiscard]] virtual huxerui::Task<DiffRevertResult>
  CheckRevert(std::string diff_id) = 0;

  [[nodiscard]] virtual huxerui::Task<void> MarkReverted(std::string diff_id) = 0;

  [[nodiscard]] virtual huxerui::Task<void>
