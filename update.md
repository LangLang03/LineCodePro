...
- **会话加载批量化** - `ConversationRepository` 改为一条 JOIN 查询读完整个会话的分块文本（原来每条消息每个字段一次查询，1000 条消息即数千次 SQL），并把旧版内联列一起选出，消除逐条的 `length()/substr()` 探测；`MessageRecord.toChatMessage` 整段 `raw_json` 只解析一次（原来每条消息重复 `new JSONObject` 近十次）
- **持久化写入降载** - 重写会话消息时用一条按会话的 `DELETE` 代替「每条消息 × 每个字段」的删除，空正文不再写入分块行，后台写库持锁时间明显缩短，不再阻塞主线程的读库
- **自动备份限流** - 每次落库都触发的全量 JSON 快照备份改为最小间隔 + 在途合并（新请求合并为一次收尾备份），避免长会话期间后台 CPU/磁盘持续饱和以及与主线程争锁
- **渲染热路径去全量重算** - 上下文 token 估算（`ContextTokenMeter`）、本地评审展示列表（`ToolReviewController`）、内容净化（`MessageContentSanitizer`）全部改为按不可变消息实例增量复用，不再每 80ms 扫描/拷贝/解析整条对话正文
- **时间线增量构建** - 新增 `ConversationTimeline.Builder`，按消息分组缓存不可变 `Row`，流式输出只重建尾部分组；`Row` 自身缓存 diff 操作、变更文件与思考块归属，视图绑定时不再做 O(N) 查找与重复 JSON 解析
- **消息实例引用稳定** - 聊天列表不再为每条带工具调用的消息复制新对象（`ToolResult` 与合并结果均按实例记忆化），使时间线、token 估算与视图绑定能够真正复用
- **行视图绑定短路** - `AssistantTurnView` / `AssistantMessageView` / `ToolCallBlockView` 在行与输入未变化时直接跳过重绘，工具卡与思考块的「拼接全文做签名」改为比较不可变实例引用；`reconcile` 由 O(n²) 改为哈希集合判定
- **滚动路径降载** - 滚到底按钮只在显示状态真正变化时更新（不再每帧 `bringToFront`），按钮样式仅在主题变化时重建；同一帧内多次 render 合并为一次滚到底；行视图缓存上限由 140 收紧到 64
- **图片降采样解码** - Markdown 本地图片先读边界再按 `inSampleSize` 解码，与 data URI 路径一致，避免整分辨率解码造成掉帧与 GC 抖动

### 原生 UI 与聊天布局重写

- **原生界面重构** - 重新整理聊天页、设置页、抽屉、底部弹窗和工具卡片的布局，统一页面底色、卡片层级、圆角、字体与图标比例；继续使用 Android 原生 View，减少重复边框、过大的标题和占用空间过多的装饰组件
- **聊天导航恢复与调整** - 左上角保留三条杠菜单，点击工作区标题打开工作区选择抽屉；恢复权限选择器与三点菜单，新对话使用加号图标，附件加号继续用于选择文件，设置与会话导航保留在抽屉中
- **用户消息与输入框** - 用户消息改为右对齐气泡，区别于助手正文和过程说明；收紧输入框默认高度、内部留白与底部操作区，并将气泡左右外边距收紧到 16dp
- **统一页头操作尺寸** - `ScreenHeaderView` 统一返回及图标操作按钮的 36dp 操作区域与 22dp 图标尺寸，修复错误日志、添加模型等页面右上角按钮过大，以及保存操作贴到屏幕最右侧的问题
- **自适应页面与弹窗** - 新增 `ScreenSurfaceView`、`InsetSheetLayout`、`AdaptiveActionsView` 等共享布局组件；宽屏限制内容宽度，底部弹窗限制最大高度，长内容可以滚动，窄屏操作按钮可以重新排列
- **Markdown 排版调整** - 调整正文、标题、代码块的字号和间距，代码区域使用更紧凑的显示方式；保留链接、复制、代码换行等阅读操作
...
- **错误与重试归入同一轮** - 中途模型错误、重试说明折叠在同一个处理区域中，避免一个对话回合出现两个已处理块；重试消息标记为上下文排除项，保留用于界面展示与历史恢复，过期重试回调不再继续已结束的生成
- **压缩过程归入当前回合** - 自动上下文压缩继承当前处理计时，运行、完成及失败状态直接显示在同一个「处理中 / 已处理」区域；压缩项不再伪装成「使用了 1 个工具」的二级折叠，完成后继续生成时不会新增第二个处理区
- **处理区域展开崩溃修复** - 时间线为重复或缺失的工具调用 ID 分配稳定且唯一的显示键，并按父容器实时子项数安全重排 View，修复 Android 10 展开「已处理」时 `addView(index)` 越界崩溃（#28）

### 工具调用与文件差异展示

- **读取工具简化** - 文件读取成功时使用轻量结果行，不再提供点击展开原始读取内容的入口；失败结果仍在调用内部提供错误详情
- **Shell 与通用工具卡片** - 调整执行命令、状态和输出的层级，MCP 摘要行与读取、Shell 工具统一为 48dp 高度、相同图标比例及无背景样式；展开后使用带边框的代码结果面板查看输入和返回内容，长输出使用有界滚动容器，避免单次调用撑满聊天页面
- **编辑与创建文件展示** - 重写 `ToolCallWriteView`、`DiffView`，按文件名、增删统计和代码差异组织内容；新增 `DiffLines` 处理差异行，新增与删除使用独立底色、行号及增删标记，修正文件列表缩进和差异内容的对齐
- **文件变更汇总** - 在答复后汇总编辑过的文件及增删数量，保留审阅、撤销和展开文件列表的入口；过程中的调用详情与末尾文件汇总分别维护展开状态
- **错误信息内联** - Shell、读取、写入、删除、Todo 等工具错误放入对应调用结果内部，展开后可查看完整错误文本，不在调用卡片外重复显示错误横幅
- **网页工具图标区分** - 网页搜索使用放大镜图标，查看网页使用圆形地球图标；读取类工具卡片读取工具自身的动作图标，不再统一显示文件图标
- **工具错误日志补齐** - `ToolExecutor` 对未知工具、工具返回失败以及执行异常记录错误日志；`ErrorLogRepository` 使用独立临时文件名避免同一时间写入互相覆盖，并同步保护时间格式化器，保留已有日志脱敏

### 执行授权与权限模式

- **底部执行申请卡** - 新增 `ToolApprovalView`，将执行说明、命令和授权操作集中到输入区附近，调整按钮尺寸、间距及窄屏排列，提供拒绝、允许一次与永久允许操作
- **永久允许精确匹配** - 新增 `CommandPermissionRepository`，按执行范围、工具名、完整 Shell 命令与工作目录生成授权键并持久化；相同命令在相同范围内可复用授权，命令或目录变化后不会误用旧授权
- **自动权限生效修复** - 自动模式下，已启用工具直接执行，主流程和 Agent 内部执行都不再重复弹出确认；确认模式继续按需申请，只读限制与工具启用状态仍然生效
- **权限提示词同步** - 工具提示词明确区分自动与确认模式，要求模型直接提交工具调用，由应用决定是否弹出申请；等待授权期间切换执行范围时拒绝旧范围的申请，避免跨工作区复用待处理授权
- **确认状态与对话解耦** - 文件改动的接受、撤销及错误说明写入本地 Diff 记录，仅在 UI 渲染副本中叠加，不再改写或重新保存模型对话；执行前确认不再插入空的 `pending/accepted` 临时结果，避免授权操作中断当前工具循环并保持请求缓存前缀稳定

### 设置、主题与抽屉细节

- **完整设置树结构恢复与配色更新** - 设置主页及模型、AI 行为、工具与执行、输入输出、主题、安全、数据、扩展、Skill Hub、存储、SSH、Termux、关于等全部子设置恢复 UI 重构前的页头、内容宽度、分区卡片、表单、行内说明、彩色图标底板和分割线；继续使用新版 `LineTheme` 配色及内置主题色阶
- **主题编辑器保留完整操作** - 自定义主题继续使用完整的颜色编辑布局，保留已有配置入口，并补齐保存按钮右侧留白；浅色与深色系统栏颜色随主题适配
- **模型管理布局** - 缩小本地模型入口图标，调整预设提供商间隔和页面边距；模型选择短列表按内容高度展开，长列表受最大高度限制，模型长按菜单去除多余底部留白
- **工作区与目录抽屉** - 工作区选中背景铺满抽屉内容宽度，保留工作区选择与长按删除；目录选择确认操作置于面板内，限制窄屏和横屏下的面板尺寸
- **工具与执行页面** - 收紧页面左右边距，强化执行目标的选中状态；保留工具分组开关，删除用于展示内部工具 ID 的「工具设置」折叠项
- **存储管理显示修复** - 为刷新图标补充尺寸测量，修复无尺寸约束的按钮挤掉页面内容、只剩中央按钮的问题；恢复总占用、Diff 缓存、聊天记录、配置文件和 Home 目录的统计布局，调整卡片背景与间距
- **处理区域显示偏好** - 在显示设置新增「自动展开处理」开关；开启后新出现的处理区域自动展开，用户手动收起后不会被后续流式刷新再次打开
- **Skills 商店布局** - 商店标题移动到返回箭头右侧，发现社区 Skills、搜索框及后续内容统一使用 16dp 左右边距；扩展主页各入口之间增加 12dp 间隔
- **上下文占用入口** - 顶部盾牌左侧新增环形进度指示，随当前会话与模型上下文快照更新；点击可查看已使用 Token、模型上下文上限和占用百分比，80%% 以上使用警示色

### Skill Hub 技能商店集成

- **社区技能发现** - 新增原生 Skills 商店，支持按名称、描述或作者搜索、分页浏览，以及按热门下载、最多收藏、最近更新排序；展示技能图标和基本信息，可从扩展页面进入
- **技能详情页** - 新增概览、文件、评论、版本、评测与预览标签，展示作者、分类、说明文档、文件清单、版本信息及服务端提供的评测数据；显示脚本、API Key 需求和安全报告提示，便于安装前检查
- **文档与文件预览** - 在应用内阅读 `SKILL.md` 和可预览的文本文件；`MarkdownView` 新增可选双指缩放与文字缩放状态，支持 0.5～1.6 倍阅读比例，详情文档可开启代码换行
- **下载安装接入** - 新增 `installSkillFromSkillHub`，支持按技能与版本下载 ZIP 并交给现有技能安装流程；通过 `ExtensionStore`、扩展管理控制器和页面工厂接通入口、安装位置选择及安装后列表刷新
- **临时安装流程统一** - URI、GitHub 与 Skill Hub 安装共用 `installTemporarySkill`；无论安装成功或失败都清理对应临时来源，二进制文件写入集中到 `SkillFileManager`
- **账号登录与社区操作** - 通过内置 WebView 打开 Skill Hub 官方登录页，检测已有登录会话；原生详情支持收藏、取消收藏、发表评论、回复、点赞及删除评论，登录过期时给出对应提示
- **社区中心与发布入口** - 新增账号及社区管理入口，对需要官方网站处理的页面使用内置 WebView；支持选择本地 Skill，填写标识、显示名称和版本，通过 multipart 上传发布，SSH Skill 不作为本地发布源
- **上传与请求边界检查** - 发布前校验技能标识、版本和文件范围，限制文件数量、单文件及总上传大小，检测到密钥、凭据等敏感文件时拒绝发布；会话 Cookie 检查长度和换行，下载、图标及文件请求保留各自的响应限制
- **HTTP 二进制请求支持** - `SimpleHttpClient.Request` 新增 `bodyBytes`，上传时按实际字节长度写入请求体，支持 multipart 文件内容，避免将二进制文件先转换成字符串

### 国际化与依赖整理

- **Skill Hub 三语言资源** - 将商店、详情、登录、发布、社区中心及客户端错误提示中的硬编码文案迁入资源；补齐英文、中文与俄语各 258 条 Skill Hub 相关字符串
- **聊天和工具文案同步** - 为处理分组、文件变更汇总、授权按钮及错误展示补充英文、中文、俄语资源，并修正模型管理、SSH 设置等页面遗留的硬编码文案
- **资源提供者注入** - 新增可复用的 `ContextResourceProvider`，由 UI / 依赖组装层创建并注入 `SkillHubClient`、`SkillHubSessionClient` 等服务；服务通过 `ResourceProvider` 获取错误消息，减少对 Android `Context` 的直接依赖

### 提示词前缀与缓存复用优化

- **固定规则与动态状态分离** - `ModelPromptController` 保持基础规则、模型身份、工具说明与扩展说明组成稳定的系统前缀；当前记忆、检索历史、待办状态和本次工具可用性作为应用上下文追加到消息末尾，避免每次状态变化都改写系统提示词和已有消息前缀
- **附件归属到原用户消息** - 文件路径上下文跟随其所属用户消息追加，不再汇总到系统提示词；新增附件不会修改先前用户回合，保留原生图片输入 JSON，与上下文窗口选择一起处理
- **动态内容计入预算** - 系统规则、运行时上下文和附件说明都计入上下文预留 Token；工具次数耗尽时仍移除原生工具定义，并在末尾上下文说明本次限制，不因缓存优化绕过调用上限
- **工具与扩展稳定排序** - 原生工具请求按工具名排序；文本工具说明中的分组、工具名和扩展列表固定顺序，已启用 Agent、HTTP MCP 与 Skills 的提示词按稳定标识排序，减少注册或发现顺序变化引起的前缀差异
- **JSON 序列化顺序固定** - 新增 `StableJson`，递归排序对象键，用于模型请求体与工具参数 Schema；保留数组顺序、字符串内容和数值语义，同一结构不会因对象插入顺序不同生成不同文本
- **模板单次替换** - `StringTemplate` 改为扫描原模板一次并替换占位符，不再遍历 Map 反复替换全文；插入文本中的 `{{...}}` 保持字面内容，避免替换顺序不稳定或误展开记忆中的模板片段
- **配置变更仍即时生效** - 工作区、用户自定义提示词、权限或学习模式改变时重新构建对应规则；明确区分应用背景数据与新用户请求，手动记忆在学习模式关闭时仍可提供给模型

### 上下文压缩循环修复

- **压缩后重置用量基线** - 摘要生成请求使用的是压缩前完整对话，其输入 usage 不再作为压缩后的上下文占用；成功后重置 `TokenUsageTracker`，回退到排除旧消息后的本地估算，修复刚压缩完又立即进入压缩的循环
- **保留消息顺序与上下文身份** - `ContextCompactionController` 仅将被摘要替代的基础消息标记为 `excludeFromContext`，需要保留的用户消息维持原有位置，不再统一移到尾部；摘要继续进入模型上下文
- **GLM 思考强度兼容层** - 请求发送前按模型能力转换统一思考档位：GLM 的关闭/低映射为 `low`，自动/中/高映射为 `high`，最大映射为 `max`；普通聊天、Agent 与上下文摘要请求共用映射，压缩流程固定使用的关闭档位也会转换为 GLM 可接受的 `low`

### 终端提供者与工程配置

- **终端提供者界面调整** - 新增 `TerminalTheme`，重排服务绑定、测试命令、Provider 信息和日志操作，统一浅色 / 深色背景、按钮与日志区样式，并补充中英文资源
- **临时预览提交过滤** - `.gitignore` 新增 `/temp/`，本地 HTML 演示及 UI 核对页面不再进入源码提交
- **原生布局测试基础设施** - 引入 Robolectric `4.16`，开启 Android 资源单元测试和所需 JVM 配置，支持使用实际 Android View 生成原生渲染截图

### 测试

- 新增 `ChatLayoutTest`、`SecondaryScreenLayoutTest`、`UiCorrectionsTest`，覆盖聊天操作、设置导航、表单草稿、模型选择、工作区与目录抽屉、卡片间距、主题、窄屏、大字体及宽屏布局，并生成原生界面核对截图
- 新增 `ConversationTimelineTest`、`ProcessingStateTest`、`DiffLinesTest`，覆盖连续工具分组、思考顺序、Agent 展示、错误重试合并、计时冻结与持久化，以及代码差异行解析
- 补充压缩与处理回合合并、压缩项直接展示、上下文用量快照复制和 Android 10 重复工具 ID 展开回归测试，覆盖 #28 的越界路径
...
- 本节汇总自上个发布 tag `v1.2.6` 到当前版本的变更，包含期间 `1.2.7-rc.1`、`1.2.7-rc.2` 的更新
- 版本号升级到 `1.2.8-max`
- `versionCode` 从 `31` 升级到 `32`
- Release 导出文件名随版本统一为 `LineCode Pro 1.2.8-max.APK`
...
- **`ToolCallCardView.updateContent` 增量钩子** - 接口新增 `default updateContent(call, result)`（默认回退 `bind`，完全向后兼容），为卡片提供「仅内容变化时增量更新」的契约
- **`ToolCallBlockView` 结构/内容签名拆分** - 原 `signature()` 包含完整 `result.content`，流式输出每帧内容变化都会 `removeAllViews` + 重建整棵卡片视图树（几十个 View）；现拆分为 `structureSignature`（卡片类型 / 状态 / 参数）与 `contentSignature`（正文）：结构变 → 重建，仅内容变 → 调用已存 `childView.updateContent(...)` 增量更新，不再销毁重建
- **`ToolCallReadView` / `ToolCallGenericView` 增量更新** - 缓存消息行 / 输出 section 的 TextView，流式时仅 `setText`；`computeTerminalStatus` 由内容空/非空推导（UNKNOWN↔SUCCESS），状态切换或换行溢出切换滚动容器时回退完整绑定，保证图标/颜色过渡不变
- **`ToolCallAgentView` 增量更新** - 新增纯静态 `layoutSignature`（progressStatus / reviewState / isError / agentId / toolCount / thinking / output 存在性 / name / type / nested 数量）判定结构：签名不变时仅更新 thinking 与输出 Markdown，签名变化（如 running→done）回退完整绑定保留进度条→图标、pills 过渡
- **`ToolCallAgentPipelineView` 保守增量** - summary 计数与状态未变时仅更新汇总 Markdown；有 agent 行的场景保守全量重建（行为不变，性能优先场景为 agents 空时的汇总输出）
- **`BaseToolCallView.addMessageRow` 返回值** - 由 `void` 改为返回创建的 `TextView`，供卡片捕获并增量更新文本
