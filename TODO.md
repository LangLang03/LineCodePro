...
### C. 视觉 / 交互
- [ ] **全局不应出现按钮点击阴影/涟漪**
- [x] **未处理系统栏 inset —— 已用像素证据定位**（第 41 轮）：
      新建 `tools/ui_region_diff.py`（把差异切成连通区域，再用控件树给每个区域
      贴"这里是什么文字"），首次运行即暴露 MAE 完全掩盖的问题：
      · `composer_context_sheet` / `more_sheet` / `permission_sheet`
        **x0-1079 y2274-2399 差异 54069px**；
        `drawer_chats` / `drawer_files` **x945-1079 y2274-2399 差异 17010px**。
        **y=2274-2399 即屏幕底部 125px（导航栏/手势区）**。
      · 像素取样：该带基线为 `(241,241,241)`，候选为 `(193,194,196)`
        ——候选在导航栏区域画了更深的表面。
      · 代码定位：`SafeAreaInsets()` **全库只用了 1 次**（`drawer.cpp:248` 抽屉底部）；
        `drawer.cpp:79 HeaderTopPadding()` 在 Android 上**硬编码 `4.0F`**，
        **从不读取真实状态栏高度** → 对应"没有处理顶部状态栏"。
        `SafeAreaPadding{}`（会消费真实 inset）用了 36 处、页面正常，
        但 `main_screen.cpp:163` 把它加在了**遮罩层**
        （`Background(Color::Rgb(0,0,0,0.60F))`）上而非弹层内容上。
      **待修**：① 顶部改为读真实 inset；② 弹层的 `SafeAreaPadding` 移到内容容器；
      ③ 抽屉顶部同理。
- [x] **另外三处被 MAE 掩盖的真实缺陷（同一工具发现）**：
      · `input_settings`：基线有分区标题 **`回车键逻辑`**，候选缺失
        （x372-581 y529-562，3412px）；
      · `llm_settings`：基线有滑块标签 **`中`**（x396-570 y1036-1065，2926px）
        与 **`高`**（x310-484 y1207-1236，2820px），候选缺失或错位；
      · `keep_alive`：基线有 **`Wake Lock`** 文字，候选缺失（两处）；
      · `model_add_local`：候选多出 **`加速`**（x529-854 y1388-1417，4273px），基线没有；
      · `model_add_custom`：**同一位置基线显示输入框占位符
        （`https://api.example.com/v1` / `如 128K 或 128000`），
        候选显示的是字段标签（`Base URL` / `上下文大小`）**
        ——不只是"占位符不渲染"，而是**布局不同**。
...
- [x] **`工具与执行` 页的像素差异已定性为有意为之**（第 39 轮）：
      ...
      该页因此在像素上**不可能一致**。
      **处理（第 40 轮定稿）**：屏蔽 **y ≥ 1180** 这一段
      （最后一行共有内容是 `分派 Agent 处理任务`：基线 y=1092 / 候选 y=1112；
      首个分歧行是基线的 `手机控制` y=1259），
      上半页（标题、执行目标三项、说明、文件操作、Agent）**仍参与像素对比**。
      **效果精确印证了诊断**：MAE **7.39 → 3.51**、差异 18.29% → 6.99%，
      与同类设置页齐平（`llm_settings` 3.62 / `settings` 3.01）——
      说明超出部分**全部**来自被有意删除的那两行。
      至此 **23 个场景全部参与像素对比，无关闭项**。
